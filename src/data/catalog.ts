...
export type CertificateDesign = {
  ...
  photo: string;
  /**
   * Безопасная зона под текст — точные границы внутренней золотой рамки на
   * фото, в процентах от размеров карточки (найдены попиксельным
   * сканированием, см. отчёт задачи об увеличении карточки): top — сразу
   * под разделительной линией/лого, bottom — с отступом перед нижней
   * замыкающей линией рамки, centerX/width — между левой и правой золотой
   * линией рамки, с небольшим отступом внутрь, чтобы текст не касался линий.
   * У каждого фото рамка сдвинута по-своему (декор на фото не симметричен),
   * поэтому центр и границы — на дизайн, а не общие для всех трёх.
   */
  archBox: { top: number; bottom: number; centerX: number; width: number };
  /**
   * Цвет текста внутри рамки (номинал, "бессрочный" и т.п.) — зависит от
   * фона фото: "dark" — тёмно-золотой текст для светлого бежевого фона
...
export const designs: CertificateDesign[] = [
  {
    ...
    // Золотая рамка (сканирование standard.png): левая линия x≈9.4%, правая
    // x≈88.6%, разделительная линия заканчивается к y≈41%, нижняя
    // замыкающая линия рамки — y≈99.7%. top/bottom/centerX/width ниже —
    // с отступом внутрь от самих линий (~4%), чтобы текст их не касался.
    archBox: { top: 41, bottom: 94, centerX: 49, width: 71 },
    textTone: "light",
  },
  {
    id: "for-her",
    ...
    // Рамка for-her уже (мешают цветочные углы) — левая линия x≈15.2%,
    // правая x≈84.8%.
    archBox: { top: 41, bottom: 94, centerX: 50, width: 61 },
    textTone: "dark",
  },
  {
    id: "for-him",
    ...
    // Рамка for-him: левая линия x≈10.1%, правая x≈89.0%.
    archBox: { top: 41, bottom: 94, centerX: 50, width: 71 },
    textTone: "light",
  },
];
